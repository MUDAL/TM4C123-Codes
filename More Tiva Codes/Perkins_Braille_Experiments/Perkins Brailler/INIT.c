...
void Init(){
	...
	GPIOE->DIR &= 0x00; //Configure all port E pins as inputs
	GPIOE->DEN |= 0xFF; //Enables digital functions for PE7 to PE0
	GPIOE->AFSEL &= 0x00; //Disable alternate functions
	
	//=== PORT F DEBUG===//
	GPIOF->DIR |= 0x0E; //Configure PF3-1 as outputs (for debugging purposes)
	GPIOF->DEN |= 0x1F; //Enable digital functions for port F
	//=================//
  ...
  TIMER0->TAILR = 800000 - 1; //40ms delay between multiple button presses
	...
	GPIOE->IS &= ~0xFF; //make PE0-7 edge sensitive
	GPIOE->IBE &= ~0xFF; //disabling both edge interrupts for PE0-7
	GPIOE->IEV &= ~0xFF; //falling edge detection for PE0-7
	GPIOE->ICR |= 0xFF; //clear any prior interrupt
	GPIOE->IM |= 0xFF; //unmask interrupt
	
	NVIC->IP[4] = 1<<5; //Set priority 1 for port E interrupts
	NVIC->ISER[0] = 0x10; //enable IRQ4
	__enable_irq();
	//=================================================================
