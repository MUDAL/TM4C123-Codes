...
The ADC's interrupt is given a higher priority over the timer's interrupt.
Once the timer initiates the sampling, the ADC handler gets executed.
*/
...
void ADC_Init(){
	...
	*/
	...
	NVIC->IP[17] = 2<<5; //set priority 2 for IRQ17
}
...
void Timer_Init(){
  ...
  TIMER0->CTL |= 0x01; //Enable the timer  to start counting
	...
	NVIC->IP[19] = 3<<5; //set priority 3 for IRQ19
}
